...
class UIModel:
    def __init__(self, width, height, colors, fonts, log_file):
        ...
        self.img_path = os.path.join("assets", "IONIQ6.png")
        print("CWD =", os.getcwd())
        print("Try loading:", os.path.abspath("assets/IONIQ6.png"))
        self.top = TopBar(width, 36, colors, self.font, self.tiny_font, self)
        self.side = SidePanel(300, 36, 64, colors, height, self)
        self.side.load_image(self.img_path)
        ...
        while running:
            ...
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    ...
                    # 1) 하단바 / Back 버튼 처리
                    #    → Button.trigger가 "로그 먼저, 액션 나중"으로 처리
                    for b in self.bottom.buttons + [self.back_btn]:
                        if b.is_pressed:
                            b.is_pressed = False
                            if b.check_click(e.pos):
                                handled = True
                                b.trigger(self, e.pos)  # 여기서 logger.log + action
                            break  # 버튼 클릭 루프 탈출

                    # 2) Back/하단바 버튼 외에는 현재 화면의 on_click()으로 전달
                    if not handled and hasattr(self.current, "on_click"):
                        result = self.current.on_click(e.pos)
                        if result:
                            handled = True
